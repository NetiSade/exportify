...
class PlaylistTable extends React.Component {
  ...
  handlePlaylistSearch = async (query) => {
    if (query.length === 0) {
      ...
      const playlists = await this.playlistsData
        .search(query)
      ...
      if (playlists.length === this.playlistsData.SEARCH_LIMIT) {
        this.setSubtitle(
          `First ${playlists.length} results with "${query}" in playlist name`
        );
      } else {
        this.setSubtitle(
          `${playlists.length} results with "${query}" in playlist name`
        );
      }
    }
  };
    ...
    try {
      ...
      this.setState(
        ...
        () => {
          ...
          const max = Math.min(
            min + this.PAGE_SIZE - 1,
            this.state.playlistCount
          );
          this.setSubtitle(
            `${min}-${max} of ${this.state.playlistCount} playlists for ${this.userId}`
          );
        }
      );
    } catch (error) {
  ...
  handlePageChanged = (page) => {
    try {
      this.setState({ currentPage: page }, this.loadCurrentPlaylistPage);
    } catch (error) {
      apiCallErrorHandler(error);
    }
  };
        ...
        <div id="playlists">
          <div id="playlistsHeader">
            <Paginator
              ...
              totalRecords={this.state.playlistCount}
              onPageChanged={this.handlePageChanged}
            />
            <PlaylistSearch
              onPlaylistSearch={this.handlePlaylistSearch}
              onPlaylistSearchCancel={this.handlePlaylistSearchCancel}
              ref={this.playlistSearch}
            />
            <ConfigDropdown
